...
const Donation = props => {
	...
	const { getAidDetails } = useContext(AidContext);
	...
	const fetchProjectDetails = (projectId, agencyDetails) => {
		getAidDetails(projectId)
			.then(res => {
				...
				if (res.campaignId) fetchCampaignDetails(agencyDetails.agency.email, res);
			})
			.catch(err => {
								...
								donationList.map((d, i) => {
									return (
										<tr key={d.id}>
											...
											<td>
												{dottedString(d.amount)} {d.amount ? 'BNB' : ''}
											</td>
