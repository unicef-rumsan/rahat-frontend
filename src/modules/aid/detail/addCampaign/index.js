...
export default function AddCampaign({ match }) {
	...
	const checkUserExists = async token => {
		return new Promise((resolve, reject) => {
			...
			const checkUserResponse = axios
				.post(checkUserUrl, requestBody, { headers: { authorization: `Bearer ${token}` } })
				.then(res => {
	...
	}, [fetchAgencyUserDetails, fetchProjectDetails]);
	...
	function fetchAgencyDetails() {
		return new Promise((resolve, reject) => {
			...
			const agencyDetailsResponse = axios
				.get(agencyDetailsUrl)
				.then(res => {
					if (res.statusText === 'OK') {
						resolve(res.data);
					}
	...
	async function createCampaign(formData, token) {
		try {
			setLoading(false);
			const resData = await axios.post(`${API.FUNDRAISER_CAMPAIGN}/add`, formData, {
				headers: { Authorization: `Bearer ${token.data}` }
			});

			const campaignToProject = await axios.post(
				`${API.PROJECTS}/${projectId}/addCampaign`,
				{
	...
	const registerFundraise = async saveAsDraft => {
		...
		formData.append('story', JSON.stringify(convertToRaw(value.story.getCurrentContent())));
								...
								<Form style={{ color: '#6B6C72' }}>
									...
									<FormGroup>
										<label htmlFor="formFileSm" className="form-label mt-3">
											Upload photo that best defines your fundraiser campaign
										</label>
										<Input
									...
									<FormGroup>
										...
										{wallets?.map((wallet, index) => (
											<p className="mb-0" key={index}>
												...
												<span className="text-danger c-p" onClick={() => removeWallet(index)}>
